...
(function () {
    ...
    `;
        ...
        // Main render function for pensum
        window.renderPensumConfig = () => {
            const container = document.getElementById('pensum-grid-container');
            if (!container) return;

            // Get list of specialties to show
            let specialtiesToShow = [];
            if (currentUser.id === 'admin') {
                specialtiesToShow = Object.keys(moduleStructure);
            } else {
                // For instructors, show only what pertains to their area
                const instructorArea = currentUser.id; // 'design' or 'multimedia'
                specialtiesToShow = Object.keys(moduleStructure).filter(spec => getAreaFromSpecialty(spec) === instructorArea);
            }

            // Apply filter if active
            if (currentPensumFilter !== 'all') {
                specialtiesToShow = specialtiesToShow.filter(spec => spec === currentPensumFilter);
            }

            if (specialtiesToShow.length === 0) {
                container.innerHTML = `
                    <div class="pensum-empty">
                        <i data-lucide="book-open"></i>
                        <h3>No hay módulos disponibles</h3>
                        <p>No se encontraron módulos permitidos para tu perfil.</p>
                    </div>
                `;
                if (window.lucide) lucide.createIcons();
                return;
            ...
            // If Admin, help organize with headings
            if (currentUser.id === 'admin' && currentPensumFilter === 'all') {
                const groups = {
                    'design': { name: 'ÁREA DE DISEÑO', specs: [] },
                    'multimedia': { name: 'ÁREA DE MULTIMEDIA', specs: [] }
                };

                specialtiesToShow.forEach(spec => {
                    const area = getAreaFromSpecialty(spec);
                    if (groups[area]) groups[area].specs.push(spec);
                });

                Object.keys(groups).forEach(key => {
                    const group = groups[key];
                    if (group.specs.length > 0) {
                        html += `<div style="grid-column: 1/-1; margin-top: 30px; margin-bottom: 20px; border-bottom: 2px solid #e2e8f0; padding-bottom: 8px;">
                                    <h2 style="font-size: 14px; color: #64748b; font-weight: 800; letter-spacing: 1px; display: flex; align-items: center; gap: 10px;">
                                        <i data-lucide="${key === 'design' ? 'palette' : 'clapperboard'}" style="width:18px;"></i>
                                        ${group.name}
                                    </h2>
                                 </div>`;
                        group.specs.forEach(spec => {
                            html += renderSpecialtySection(spec);
                        });
                    }
                });
            } else {
            ...
            if (filterContainer) {
                ...
                chips.forEach(chip => {
                    ...
                    if (onclickAttr) {
                        ...
                        if (match) {
                            const cat = match[1];
                            if (cat === 'all' || currentUser.id === 'admin') {
                                chip.style.display = 'inline-block';
                            } else {
                                // Instructor: check if category belongs to their area
                                const catArea = getAreaFromSpecialty(cat);
                                chip.style.display = (catArea === currentUser.id) ? 'inline-block' : 'none';
                            }
                        }
                    }
                });
        ...
        // Helper to render a specialty section (header + cards)
        function renderSpecialtySection(specialty) {
            const modules = moduleStructure[specialty] || [];
            let sectionHtml = `
                <div class="specialty-section-header">
                    ...
                    <h2 class="specialty-title">${specialty}</h2>
                    <p class="specialty-subtitle">${modules.length} módulos · Programa completo</p>
                </div>
            `;

            let cardsHtml = '';
            modules.forEach((module, idx) => {
                const topics = pensumContent[module.name] || [];
                // Use color based on overall index would be hard here, so use module number
                const color = moduleColors[module.number % moduleColors.length];
                const deliverables = moduleDeliverables[module.name] || [];
                const moduleId = `module-card-${specialty.replace(/\s+/g, '-')}-${module.number}`;
                ...
                cardsHtml += `
                    <div class="module-card color-${color}" id="${moduleId}">
                        <div>
                            ...
                            <p class="module-description" style="font-size: 14px; font-weight: 600; margin-bottom: 8px; color: rgba(255,255,255,0.95);">${module.name}</p>
                            <p class="module-description">${module.description}</p>
                        </div>
                        <div>
                            <div class="module-stats">
                                <div class="module-stat"><i data-lucide="book-open"></i><span>${topics.length} temas</span></div>
                                <div class="module-stat"><i data-lucide="users"></i><span>${studentsInModule} alumnos</span></div>
                                ${deliverables.length > 0 ? `<div class="module-stat"><i data-lucide="clipboard-check"></i><span>${deliverables.length} entregas</span></div>` : ''}
                            </div>
                            <button class="expand-btn" onclick="toggleModuleCard('${moduleId}')">Ver temario completo</button>
